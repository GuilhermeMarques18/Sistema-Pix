...
**Biblioteca exclusiva de ícones da aplicação.**

Pacote: `@phosphor-icons/react`

Regras de uso:

- Utilizar **sempre** Phosphor Icons para qualquer ícone na aplicação.
- **Não utilizar** outras bibliotecas de ícones (ex: lucide-react, heroicons, react-icons).
- O `lucide-react` está presente no `package.json` por dependência indireta — não deve ser importado diretamente.

#### Variantes disponíveis (weight)

| Weight | Uso recomendado |
|---|---|
| `regular` | Ícones padrão / inativos |
| `bold` | Ícones de destaque ou indicadores |
| `fill` | Estado ativo (ex: tab selecionada na BottomNav) |
| `thin` | Ícones decorativos / logo |
| `light` | Uso opcional para contextos suaves |
| `duotone` | Evitar — mantém consistência visual |

#### Tamanhos recomendados

| Contexto | Tamanho |
|---|---|
| Ícone em botão de ação rápida | `24px` |
| Ícone em BottomNav | `24px` |
| Ícone inline em texto / campo | `16–20px` |
| Ícone decorativo / logo | `26px` |

#### Exemplo de uso

```tsx
import { House, ArrowLineDown } from '@phosphor-icons/react';
